...
class Game:
	...
	def __init__(self, get_next_shape, update_score):
		...
		#win
		try:
			self.win_image = pygame.image.load(join('graphics', 'win.jpg')).convert_alpha()
		except (pygame.error, FileNotFoundError):
			self.win_image = None
		self.win = False  # flag to trigger win screen
	...
	def calculate_score(self, num_lines):
		...
		if self.current_level < self.MAX_LEVEL and self.current_lines // 5> self.current_level:
			self.current_level += 1
			self.down_speed *= 0.75
			self.down_speed_faster = self.down_speed * 0.3
			self.timers['vertical move'].duration = self.down_speed
		elif self.current_level == self.MAX_LEVEL and self.current_lines // 5 > self.current_level:
			self.win = True
	...
	def run(self):
		if self.win:
			# Draw the win image
			self.display_surface.blit(self.win_image, (PADDING, PADDING))
